...
def load_real_data(subject_ids, cache_dir="./data_cache"):
    ...
    for sid in tqdm(subject_ids, desc="Loading/Preprocessing subjects"):
        night_idx = 0
        while True:
            x_path = os.path.join(cache_dir, f"subject_{
                                  sid}_night_{night_idx}_X.npy")
            y_path = os.path.join(cache_dir, f"subject_{
                                  sid}_night_{night_idx}_y.npy")
        ...
        if night_idx == 0:
            ...
            for n, (psg_path, hyp_path) in enumerate(paths):
                try:
                    p = mp.Process(target=_process_subject_night, args=(
                        sid, n, psg_path, hyp_path, cache_dir))
                    p.start()
                    p.join()

                    if p.exitcode != 0:
                        raise RuntimeError(
                            f"Preprocessing subprocess failed with exit code {p.exitcode}")

                    x_path = os.path.join(cache_dir, f"subject_{
                                          sid}_night_{n}_X.npy")
                    y_path = os.path.join(cache_dir, f"subject_{
                                          sid}_night_{n}_y.npy")
...
def main():
    parser = argparse.ArgumentParser(
        description="NeuroSleep Training Pipeline")
    ...
    parser.add_argument('--limit-subjects', type=int, default=None,
                        help="Limit number of subjects loaded (for fast testing)")
    parser.add_argument('--cache-dir', type=str, default="./data_cache",
                        help="Directory to cache preprocessed subject data")
    ...
    if args.test_mode:
        ...
        if args.limit_subjects:
            print(f"Limiting dataset load to {
                  args.limit_subjects} train subjects...")
        ...
        X_train, y_train = load_real_data(
            train_subjects, cache_dir=args.cache_dir)
        print("Loading real validation data...")
        X_val, y_val = load_real_data(val_subjects, cache_dir=args.cache_dir)

        if len(X_train) == 0 or len(X_val) == 0:
            raise ValueError(
                "No data loaded. Check connection to PhysioNet or cache directory contents.")
    ...
    train_dataset = SleepSequenceDataset(
        X_train, y_train, sequence_length=30, stride=30, is_train=True)
    val_dataset = SleepSequenceDataset(
        X_val, y_val, sequence_length=30, stride=1, is_train=False)
    ...
    csv_writer.writerow(["Epoch", "Train_Loss", "Val_Loss",
                        "Val_Macro_F1", "Val_Kappa", "Val_Acc"])

    for epoch in range(1, args.epochs + 1):
        print(f"\nEpoch {epoch}/{args.epochs}")
        train_loss = train_one_epoch(
            model, train_loader, criterion, optimizer, device, scaler)
        val_loss, macro_f1, kappa, acc, targets, preds = evaluate(
            model, val_loader, criterion, device)
        scheduler.step()

        print(f"Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f}")
        print(f"Val Macro F1: {macro_f1:.4f} | Val Kappa: {
              kappa:.4f} | Val Acc: {acc:.4f}")

        # Log to CSV
        csv_writer.writerow(
            [epoch, train_loss, val_loss, macro_f1, kappa, acc])
        log_file.flush()
    ...
    report = classification_report(targets, preds, target_names=[
                                   'Wake', 'N1', 'N2', 'N3', 'REM'], zero_division=0)
    print(report)
